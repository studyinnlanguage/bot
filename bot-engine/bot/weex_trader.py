...
class WEEXFuturesTrader:
    ...
    def _retry_api_call(self, func, *args, max_retries=3, **kwargs):
        """Retry an API call on transient network errors.

        Only retries on network-related errors (timeouts, connection resets,
        SSL issues, etc.). Business-logic errors (auth failures, invalid
        parameters) are raised immediately.
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    logger.warning('WEEX API call failed (attempt %d/%d), retrying in %ds: %s',
                                  attempt + 1, max_retries, wait_time, e)
                    _time.sleep(wait_time)
                else:
                    logger.error('WEEX API call failed after %d attempts: %s', max_retries, e)
        raise last_error
    ...
    def get_klines(self, symbol: str, interval: str = "1d", limit: int = 200) -> pd.DataFrame:
        ...
        data = self._retry_api_call(self._request, "GET", path, params=params)
    ...
    def get_balance(self) -> float:
        ...
        try:
            ...
            data = self._retry_api_call(self._request, "GET", path, signed=True)
        except Exception as e:
    ...
    def place_market_order(self, symbol: str, side: str, quantity: float,
        ...
        try:
            ...
            resp = self._retry_api_call(self._request, "POST", path, body=body, signed=True)
    ...
    def compute_quantity(self, notional_usdt: float, price: float,
        ...
        if qty_step > 0:
            ...
            decimals = max(0, int(math.ceil(-math.log10(qty_step)))) if qty_step >= 1e-10 else 8
