...
class Notifier:
    ...
    def _validate_telegram(self) -> Optional[str]:
        ...
        if ":" not in self.tg_bot_token:
            return "Bot Token format is invalid (colon ':' missing). Example: 123456789:ABCdefGHIjkl"
        parts = self.tg_bot_token.split(":", 1)
        if not parts[0].isdigit() or len(parts[0]) < 5:
            return "Bot Token numeric part is invalid"
        if len(parts[1]) < 20:
            return "Bot Token secret part is too short (likely incomplete)"
        if not self.tg_chat_id:
            return "Chat ID is empty"
        if not self.tg_chat_id.lstrip("-").isdigit():
            return "Chat ID must be numbers only (with optional leading '-')"
        return None

    def _validate_email(self) -> Optional[str]:
        """Validate Email config."""
        if not self.email_sender:
            return "Sender email is empty"
        if "@" not in self.email_sender:
            return "Invalid sender email format"
        if not self.email_password:
            return "App password is empty"
        if not self.email_receiver:
            return "Receiver email is empty"
        if "@" not in self.email_receiver:
            return "Invalid receiver email format"
        return None
    ...
    def _validate_whatsapp(self) -> Optional[str]:
        ...
        if not self.whatsapp_phone.isdigit():
            return "Phone number must be digits only (with country code, e.g. 923001234567)"
    ...
    def _send_telegram_sync(self, message: str, results: dict):
        ...
        try:
            ...
            err = "Timeout - check your internet or Telegram may be blocked"
            logger.error("Telegram timeout: %s", err)
            results["telegram"] = {"success": False, "error": err}
        except requests.exceptions.ConnectionError as e:
            err = f"Connection error - check your internet: {e}"
    ...
    def _send_email_sync(self, subject: str, body: str, results: dict):
        ...
        try:
            ...
            err = f"Auth fail - App Password is incorrect: {e}"
    ...
    def _send_whatsapp_sync(self, message: str, results: dict):
        ...
        try:
            ...
            err = "Timeout - check your internet"
    ...
    def notify_signal(self, symbol: str, signal: str, reason: str, emas: dict):
        ...
        msg = (
            ...
            f"⚡ EMA55 crossover just detected"
        )
    ...
    def notify_tp(self, symbol: str, side: str, entry_price: float, close_price: float,
                  pnl_pct: float, emas: dict, tp_type: str = "fixed"):
        """Send TP hit notification.

        Args:
            tp_type: "fixed" for fixed percentage TP, "ema_reversal" for EMA reversal TP.
        """
        if tp_type == "ema_reversal":
            reason_line = "EMA55 reversal triggered (EMA55 flipped to opposite extreme)"
        else:
            reason_line = "Take Profit target reached"
    ...
    def notify_bot_stop(self):
        ...
        return self.send("Bot Stopped", "Bot has been stopped by the user.")
